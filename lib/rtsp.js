...
function checkrawdata(data) {
  ...
  // return text
  return text.replace('aO4Gcg==','aO4CXIA=');

  // return text.replace('aO4FcsA=','aO4GcsA=');
  // return text.replace('aO4GcsA=','aO4FcsA=');
}

function parseUrl(string) {
  const parse = url.parse(string)
  const host = parse.host;
  const port = parse.port || 554;
...
class Rtsp extends EventEmitter {
  ...
  tcpserver_message(data) {
    ...
    if (data.length > 4 && data.readUInt16BE(2) <= data.length) {
      const l = data.readUInt16BE(2) + 4;
      if (data[4] === 160) {
        this.emit('stream', { id: this.id, data: data.slice(4, l - data[l - 1]) });
      } else {
        this.emit('stream', { id: this.id, data: data.slice(4, l) });
      }
      if (l !== data.length) {
