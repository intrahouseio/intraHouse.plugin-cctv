...
class Rtsp extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.host = '127.0.0.1' || '192.168.0.64';
    this.port = 9999;

    this.url = `rtsp://${this.host}:${this.port}/stream1`;
    this.login = 'admin';
    this.pass = '123456' || 'hikvision662412';
    this.auth = false;
    this.digestrealm = '';
    this.nonce = '';
    this.udpport1 = '';
  ...
  tcpserver_message(data) {
    ...
    if (data.length > 4 && data.readUInt16BE(2) <= data.length) {
      ...
      //  console.log(data[1])
      //  fs.appendFileSync('test.h264', Buffer.concat([new Buffer([0,0,0,1]), data.slice(16, l)]))
