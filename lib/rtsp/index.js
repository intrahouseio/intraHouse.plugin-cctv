...
class Rtsp extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.url = `rtsp://${this.host}:${this.port}/stream2` || 'Streaming/Channels/102';
    this.login = 'admin';
  ...
  tcpserver_message(data) {
    ...
    if (data.length > 4 && data.readUInt16BE(2) <= data.length) {
      ...
      if (data[4] === 160) {
        this.emit('jpeg', { id: this.id, data: data.slice(4, l - data[l - 1]) });
      } else {
        this.emit('jpeg', { id: this.id, data: data.slice(4, l) });
      }
      if (l !== data.length) {
